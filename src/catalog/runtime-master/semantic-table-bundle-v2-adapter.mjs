...
function sizeModeLabel(value) {
  if (value === 'STANDARD') return '規格';
...
function valuesFor(document, aliasRuntimeToUi) {
  ...
  for (const row of rows(document, 'size_capabilities')) {
    if (row.status !== 'VERIFIED' || row.supported !== true) continue;
    addValue(out, seen, row.mode_field_id, row.mode_value, sizeModeLabel(row.mode_value));
  }
...
function fieldDefinitions(document, aliasRows) {
  ...
  const dimensionUpstream = unique([
    ...(contract.dimension_validation?.selection_join_keys ?? []),
    'glass_family', 'glass_type', 'lowe_color', 'cavity_fill', 'supply_form', 'glass_detail', 'decorative_pattern',
  ]).map((field) => aliasRows.find((row) => row.runtime_rule_field_id === field)?.ui_field_id ?? field);
  return order.map((field, index) => {
    ...
    return {
      ...
      initial_visibility: Object.prototype.hasOwnProperty.call(fixed, field) || ['order_width','order_height'].includes(field) || ui.includes('非表示') || ui.includes('条件') || !row ? 'HIDE' : 'SHOW',
    };
  });
}
...
export function adaptSemanticTableBundleV2(runtimePackage) {
  ...
  const sizeMode = contract.size_mode_contract;
  if (!sizeMode || sizeMode.field_id !== 'size_mode' || sizeMode.custom?.supported !== true || sizeMode.standard?.supported !== false) {
    fail('SEMANTIC_BUNDLE_SIZE_MODE_CONTRACT_INVALID', 'Formal size_mode contract with CUSTOM supported and STANDARD unsupported is required.');
  }
  for (const name of [
    contract.machine_join_contract.field_alias_table,
    contract.machine_join_contract.size_capability_table,
  ...
  const sizeCapabilities = table(contract.machine_join_contract.size_capability_table);
  const supportedSizeModes = sizeCapabilities
    .filter((row) => row.status === 'VERIFIED' && row.supported === true)
    .map((row) => row.mode_value);
  if (!supportedSizeModes.length || supportedSizeModes.some((value) => !(sizeMode.allowed_values ?? []).includes(value))) {
    fail('SEMANTIC_BUNDLE_SIZE_CAPABILITY_TABLE_INVALID', 'size_capabilities does not match size_mode_contract.');
  }
  const dimensionUpstreamFields = unique([
    ...(contract.dimension_validation.selection_join_keys ?? []),
    'glass_family', 'glass_type', 'lowe_color', 'cavity_fill', 'supply_form', 'glass_detail', 'decorative_pattern',
  ]).map((field) => aliasRuntimeToUi.get(field) ?? field);
  const master = {
    ...
    capabilities: {
      ...
      uiSemanticSupport: {
        ...
        customSize: {
          modeField: sizeMode.field_id,
          allowedModes: supportedSizeModes,
          customMode: contract.dimension_validation.custom_mode_value ?? supportedSizeModes[0] ?? null,
          standardSupported: sizeMode.standard.supported,
          standardRecordSource: sizeMode.standard.candidate_source,
          customSupported: sizeMode.custom.supported,
          dimensionFields: [...(sizeMode.custom.dimension_fields ?? [])],
          unit: sizeMode.custom.unit,
          stepMm: sizeMode.custom.step_mm,
          validationResultModel: [...(sizeMode.validation_result_model ?? [])],
          clearRules: { ...(sizeMode.clear_rules ?? {}) },
          dimensionUpstreamFields,
        },
      },
    },
  };
