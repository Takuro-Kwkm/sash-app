...
function applySizeMode(master, state) {
  const cap = master.capabilities?.uiSemanticSupport?.customSize;
  if (!cap?.modeField) return;
  const mode = state.fields[cap.modeField];
  if (!mode) return;
  setAllowed(state, cap.modeField, cap.allowedModes ?? []);
  mode.required = true;
  mode.visibility = 'SHOW';
  const customActive = cap.customSupported === true && present(mode.value) && (cap.allowedModes ?? []).some((value) => Object.is(value, mode.value));
  for (const field of cap.dimensionFields ?? []) {
    const target = state.fields[field];
    if (!target) continue;
    target.unit = cap.unit ?? target.unit ?? null;
    target.step = cap.stepMm ?? target.step ?? null;
    target.required = customActive;
    target.visibility = customActive ? 'SHOW' : 'HIDE';
    if (!customActive) target.state = present(target.value) ? target.state : 'NOT_APPLICABLE';
