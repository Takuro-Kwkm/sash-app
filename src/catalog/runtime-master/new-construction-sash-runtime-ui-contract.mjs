...
function approvedNewConstructionExtension(key,field={}){if(field.domain==='INSTALLATION')return{slot:`extension:installation:${key}`,stage:'INSTALLATION_SURVEY',order:field.displayOrder};if(field.domain==='OPTION'||field.domain==='HARDWARE')return{slot:`extension:option:${key}`,stage:'OPTION',order:field.displayOrder};return null;}
export function applyNewConstructionSashUiOrder(fields=[]){return applyGlobalWindowSelectionFlow(fields,{uiCategory:NEW_CONSTRUCTION_EXTERIOR_WINDOW_UI_CATEGORY,canonicalSlotOrder:NEW_CONSTRUCTION_SASH_UI_STANDARD_ORDER,semanticSlotForField:semanticSlotForNewConstructionField,semanticStageForSlot:semanticStageForNewConstructionSlot,approvedExtensionForField:approvedNewConstructionExtension,shouldExposeField:shouldExposeNewConstructionRuntimeField,standardLabelForField:standardLabelForNewConstructionField});}
