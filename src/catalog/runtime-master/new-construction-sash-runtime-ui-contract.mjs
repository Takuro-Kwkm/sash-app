...
const exactCallCodeFromFormalSizeId=(objects)=>{for(const object of objects){if(!object||typeof object!=='object')continue;for(const key of ['sizeId','size_id','id','サイズID']){const raw=object[key];if(raw===undefined||raw===null||raw==='')continue;const tokens=String(raw).split(/[^0-9A-Za-z]+/).filter(Boolean);for(let index=tokens.length-1;index>=0;index-=1)if(/^\d{5}$/.test(tokens[index]))return tokens[index];for(let index=tokens.length-1;index>=1;index-=1)if(/^\d{2}$/.test(tokens[index])&&/^\d{3}$/.test(tokens[index-1]))return `${tokens[index-1]}${tokens[index]}`;}}return null;};
export function standardSizeMetadataFromRuntimeValue(row={}){const objects=[row.sizeMetadata,row.metadata,row.source?.metadata,row.source,row];let callCode=firstPresent(objects,['callCode','call_code','sizeCode','size_code','呼称寸法']);if(callCode===null||callCode==='')callCode=exactCallCodeFromFormalSizeId(objects);const callW=firstPresent(objects,['callW','call_w','nominalW','nominal_w','呼称W']);const callH=firstPresent(objects,['callH','call_h','nominalH','nominal_h','呼称H']);const actualWRaw=firstPresent(objects,['actualW','actual_w','実寸W']);const actualHRaw=firstPresent(objects,['actualH','actual_h','実寸H']);if((callCode===null||callCode==='')&&typeof callW==='string'&&typeof callH==='string'&&callW.trim()&&callH.trim())callCode=`${callW.trim()}${callH.trim()}`;if(callCode===null||callCode===''||actualWRaw===null||actualHRaw===null)return null;const actualW=Number(actualWRaw),actualH=Number(actualHRaw);if(!Number.isInteger(actualW)||!Number.isInteger(actualH))return null;return Object.freeze({callCode:String(callCode),actualW,actualH});}
