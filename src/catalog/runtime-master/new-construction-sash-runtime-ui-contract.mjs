...
const SLOT_ALIASES = Object.freeze({
  ...
  composition_type:'window_spec',configuration:'window_spec',variant:'window_spec',
  handing:'handing',
  size_mode:'size_mode',sash_count:'panel_count',panel_count:'panel_count',leaf_configuration:'panel_count',size:'size',standard_size:'size',size_id:'size',
  exterior_color:'exterior_color',interior_color:'interior_color',
  screen:'screen_presence',screen_presence:'screen_presence',screen_type:'screen_form',screen_form:'screen_form',screen_midrail:'screen_midrail',screen_net:'screen_net',
...
const INTERNAL_EXACT = new Set(['construction','actual_w','actual_h','runtime_technical_state','dependency_only_selector','internal_provider_id','provider_id','source_id','internal_normalized_id','normalized_internal_id']);
const isInternalKey=(key)=>{
  const token=String(key??'').trim();
  if(!token)return true;
  return INTERNAL_EXACT.has(token)||token.startsWith('_')||token.startsWith('internal_')||token.startsWith('runtime_internal_')||token.startsWith('dependency_only_');
};
...
export function shouldExposeNewConstructionRuntimeField(field={}){
  const key=field.key??field.field_name;
...
export function applyNewConstructionSashUiOrder(fields=[]){
  return fields.filter(shouldExposeNewConstructionRuntimeField).map((field,index)=>{
    const slot=semanticSlotForNewConstructionField(field.key);
    return {...field,semanticSlot:slot,displayLabel:standardLabelForNewConstructionField(field.key,field.displayLabel),__uiSlotIndex:SLOT_INDEX.get(slot)??NEW_CONSTRUCTION_SASH_UI_STANDARD_ORDER.length,__runtimeOrder:Number(field.displayOrder??1000+index),__inputIndex:index};
  }).sort((a,b)=>a.__uiSlotIndex-b.__uiSlotIndex||a.__runtimeOrder-b.__runtimeOrder||a.__inputIndex-b.__inputIndex).map((field,index)=>{
    const {__uiSlotIndex,__runtimeOrder,__inputIndex,...rest}=field;
    return {...rest,displayOrder:(index+1)*10};
  });
}

const firstPresent=(objects,keys)=>{
...
export function standardSizeMetadataFromRuntimeValue(row={}){
  const objects=[row.sizeMetadata,row.metadata,row.source?.metadata,row.source,row];
  let callCode=firstPresent(objects,['callCode','call_code','sizeCode','size_code','呼称寸法']);
  const callW=firstPresent(objects,['callW','call_w','nominalW','nominal_w','呼称W']);
  const callH=firstPresent(objects,['callH','call_h','nominalH','nominal_h','呼称H']);
  const actualWRaw=firstPresent(objects,['actualW','actual_w','実寸W']);
  const actualHRaw=firstPresent(objects,['actualH','actual_h','実寸H']);
  if((callCode===null||callCode==='')&&callW!==null&&callH!==null)callCode=`${callW}${callH}`;
  if(callCode===null||callCode===''||actualWRaw===null||actualHRaw===null)return null;
  const actualW=Number(actualWRaw),actualH=Number(actualHRaw);
  if(!Number.isInteger(actualW)||!Number.isInteger(actualH))return null;
  return Object.freeze({callCode:String(callCode),actualW,actualH});
}

export function formatStandardSizeLabel(metadata){
  if(!metadata){
    const error=new Error('Standard size display requires formal Runtime callCode/nominal dimensions and actualW/actualH.');
    error.code='RUNTIME_SIZE_DISPLAY_DATA_MISSING';throw error;
  }
  return `${metadata.callCode} ｜ W ${metadata.actualW} × H ${metadata.actualH}`;
}

export function formatAndSortStandardSizeChoices(choices=[]){
  return choices.map((choice,index)=>{
    const metadata=standardSizeMetadataFromRuntimeValue(choice.runtimeValueRow??choice);
    return {...choice,displayLabel:formatStandardSizeLabel(metadata),sizeMetadata:metadata,__stableIndex:index};
  }).sort((a,b)=>a.sizeMetadata.actualW-b.sizeMetadata.actualW||a.sizeMetadata.actualH-b.sizeMetadata.actualH||a.__stableIndex-b.__stableIndex).map(({runtimeValueRow,__stableIndex,...choice})=>choice);
}
