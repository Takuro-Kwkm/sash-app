...
export const runtimeMasterInventory = Object.freeze([
  Object.freeze({
    manufacturer: 'LIXIL', series: 'EW', masterVersion: 'v1.1', schemaVersion: '2.0', packageType: 'RUNTIME_MANIFEST_V2', adapterType: 'CANONICAL_WORKBOOK_REFERENCE_V1',
    packageRoot: EW_ROOT, runtimeManifestPath: join(EW_ROOT, 'runtime_manifest.json'), runtimeManifestDriveFileId: '139c0atou5LFz7EIHIdD7ZTYWddfSHf5_', runtimeManifestSha256: '8646bfd5f4a3d2184f2dbcb5b28f6e7dca995c9e11d53ec273a158588b5cdbed',
  ...
  Object.freeze({
    manufacturer: 'YKK AP', series: 'ウチリモ 内窓', masterVersion: 'v1.0-P7R1-R2', schemaVersion: '2.0', packageType: 'RUNTIME_MANIFEST_V2', adapterType: 'UCHIRIMO_TABULAR_V1', requireRuntimeContract: false, enforceComponentSchemaVersion: false,
    packageRoot: UCHIRIMO_ROOT, runtimeManifestPath: join(UCHIRIMO_ROOT, 'runtime_manifest.json'), runtimeManifestDriveFileId: '1119yamXn21wLZd3C8LvamNWsTAx_1dt2', runtimeManifestSha256: 'be4f1f77727424dc06ddf9de947201f33d4aee5219b182e37d0f178e1fb7147d',
  ...
  Object.freeze({
    manufacturer: 'LIXIL', series: 'TW', masterVersion: 'integrated-v0.2', schemaVersion: '2.0', packageType: 'RUNTIME_MANIFEST_V1', adapterType: 'TW_CANONICAL_WORKBOOK_REFERENCE_V1',
    packageRoot: TW_ROOT, runtimeManifestPath: join(TW_ROOT, 'runtime_manifest.json'), runtimeManifestDriveFileId: '1f9ogJ2pS0HmrUuXG1Qy431lG0mgxN9pw', runtimeManifestSha256: '52af3e462f940df67c267de5f715250290136afdd67a70611e684fcc3d5d064e',
...
async function loadRuntime(entry) {
  let runtimePackage, adapted;
  if (entry.packageType === 'RUNTIME_MANIFEST_V2' && entry.adapterType === 'CANONICAL_WORKBOOK_REFERENCE_V1') {
    runtimePackage = await loadCanonicalWorkbookRuntimePackage(entry); adapted = adaptCanonicalWorkbookReferenceV1(runtimePackage);
  } else if (entry.packageType === 'RUNTIME_MANIFEST_V2' && entry.adapterType === 'UCHIRIMO_TABULAR_V1') {
    runtimePackage = await loadCanonicalWorkbookRuntimePackage(entry); adapted = adaptUchirimoTabularV1(runtimePackage);
  } else if (entry.packageType === 'RUNTIME_MANIFEST_V1' && entry.adapterType === 'TW_CANONICAL_WORKBOOK_REFERENCE_V1') {
    runtimePackage = await loadManifestRuntimePackage(entry); const master = adaptTwCanonicalWorkbookReferenceV1(runtimePackage); adapted = { master, resolver: (selection) => evaluateCanonicalWorkbookRuntime(master, selection) };
  } else if (entry.packageType === 'RUNTIME_MANIFEST_V1' && entry.adapterType === 'SEMANTIC_TABLE_BUNDLE_V2') {
    runtimePackage = await loadManifestRuntimePackage(entry); const master = adaptSemanticTableBundleV2(runtimePackage); adapted = { master, resolver: (selection) => evaluateSemanticTableBundleV2(master, selection) };
  } else {
    const error = new Error(`Unsupported release Runtime package: ${entry.packageType}/${entry.adapterType}`); error.code = 'RUNTIME_ADAPTER_NOT_REGISTERED'; throw error;
  }
  return Object.freeze({ entry, master: adapted.master, api: null, resolver: adapted.resolver, sourcePackageIntegrity: runtimePackage.integrity, normalizedManifest: runtimePackage.manifest });
}
export async function loadRegisteredRuntime(manufacturer, series) {
  const entry = getRuntimeMasterEntry(manufacturer, series); if (!entry) return null;
