...
function validateJsonSchema(value, schema, path = '$') {
  ...
  if (Array.isArray(value)) {
    ...
    if (schema.items) value.forEach((child, index) => errors.push(...validateJsonSchema(child, schema.items, `${path}[${index}]`)));
  }
...
export function normalizeRuntimeManifest(raw) {
  ...
  const runtimeFiles = Array.isArray(raw.runtime_files) ? raw.runtime_files.map((row) => ({ role: row.role, fileName: row.file_name ?? row.name, fileId: row.file_id ?? row.id, sha256: row.sha256 })) : [];
  ...
  const normalized = {
    schemaVersion: raw.schema_version, manufacturer: raw.manufacturer, series: raw.series, packageVersion: raw.package_version,
    runtimeStatus: raw.runtime_status, runtimeFiles, schemaFile,
    formalPass: raw.formal_pass === true || raw.master_status === 'FORMAL_PASS',
    storageStatus: raw.storage_status, packageGate: raw.package_gate,
    storageGate: raw.storage_gate ?? (raw.storage_status === 'CANONICAL' ? 'PASS' : null),
    registryGate: raw.registry_gate ?? raw.registry_status ?? null,
...
export async function loadManifestRuntimePackage(entry) {
  ...
  const storageReady = manifest.storageStatus === 'PASS' || manifest.storageStatus === 'CANONICAL' || (manifest.storageStatus === 'DRIVE_CANONICAL' && manifest.storageGate === 'PASS');
  if (!manifest.formalPass || manifest.runtimeStatus !== 'READY' || !storageReady || manifest.packageGate !== 'PASS' || (manifest.registryGate && manifest.registryGate !== 'PASS')) fail('RUNTIME_MANIFEST_NOT_FORMAL_READY', 'Canonical Runtime manifest is not formally READY', { manifest });
  const schemaLoaded = manifest.schemaFile ? await readMaterializedCanonicalFile(entry, manifest.schemaFile) : null;
  const loadedByRole = new Map(), fileIntegrity = [];
  for (const row of manifest.runtimeFiles) {
    const loaded = await readMaterializedCanonicalFile(entry, row); loadedByRole.set(row.role, loaded.json);
  ...
  const schema = schemaLoaded?.json ?? null, schemaErrors = [];
  for (const [role, document] of loadedByRole.entries()) {
    ...
    for (const [name, expected] of [['manufacturer', manifest.manufacturer], ['series', manifest.series], ['package_version', manifest.packageVersion]]) if (String(meta[name]) !== String(expected)) schemaErrors.push(`$.${role}.metadata.${name}: expected ${expected}, got ${meta[name]}`);
  }
