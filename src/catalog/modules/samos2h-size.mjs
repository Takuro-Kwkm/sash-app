...
export const sizes=source.sizesPacked.map(p=>{const[n,wi,si,callW,callH,actualW,actualH,ci,cli,callCode,innerCode,fi,page,gi,patternAllowed,handingRequired,sourceRow]=p;return{id:`SZ-S2H-${String(n).padStart(5,'0')}`,window:d.windows[wi],spec:si>=0?d.specs[si]:null,callW:String(callW),callH:String(callH),actualW,actualH,construction:d.constructions[ci],windowClass:d.classes[cli],callCode,innerCode,frameType:fi>=0?d.frames[fi]:null,page,glassSymbol:gi>=0?d.glassSymbols[gi]:null,patternAllowed:Boolean(patternAllowed),handingRequired:Boolean(handingRequired),sourceRow};});
