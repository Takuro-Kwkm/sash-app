...
export function catalogInventory(catalog){return catalog.products.map(p=>{const m=r=>r.productId===p.id;return{productId:p.id,manufacturer:p.manufacturer,series:p.displayName,definitions:catalog.specificationDefinitions.filter(m).length,allowedValues:catalog.allowedValues.filter(m).length,requiredRules:catalog.requiredFieldRules.filter(m).length,ruleSets:catalog.ruleSets.filter(m).length,dependencies:catalog.dependencies.filter(m).length,evidence:catalog.evidence.filter(m).length};});}
