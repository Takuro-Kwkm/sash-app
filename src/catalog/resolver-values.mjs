...
import{sizeAllowedValues}from'./size-resolver.mjs';
import{sizeModeAvailable}from'./size-availability.mjs';
...
export function getAllowedValues(catalog,productId,key,selection={},context=buildCatalogContext(catalog,productId)){const source=key==='size'&&catalog.standardSizeRecords?.some(row=>row.productId===productId)?sizeAllowedValues(catalog,productId,selection,context):catalog.allowedValues.filter(row=>row.productId===productId&&row.specificationKey===key);return dedupe(source.filter(row=>row.status!=='INACTIVE'&&selectorMatches(row.selector,selection,context)&&!denied(catalog,productId,row,selection,context)&&(key!=='size_mode'||sizeModeAvailable(catalog,productId,row.value,selection,context))));}
