...
async function renderOpeningEditor(projectId,estimateId,openingId){
  ...
  const setSaveStatus=(next,message='')=>{status=next;statusTarget.innerHTML=saveStatusMarkup(next,message);$('#status').textContent=next==='SAVE_FAILED'?'SAVE FAILED':next;$('#status').classList.toggle('ok',next!=='SAVE_FAILED');};
  ...
  const persist=()=>{
    ...
    saveChain=saveChain.then(async()=>{
      ...
      try{
        ...
        if(request===sequence){setSaveStatus(SaveStatus.SAVED);localStorage.removeItem(draftKey(openingId));$('#saveFailureHelp').innerHTML='';}
        return true;
      }catch(error){
        if(request===sequence){setSaveStatus(SaveStatus.SAVE_FAILED,error.message);$('#saveFailureHelp').innerHTML='<div class="notice error">入力内容はこの画面に保持されています。<button type="button" class="button secondary small" data-action="retry-save">再試行</button></div>';}
        return false;
      }
    });
    return saveChain;
  ...
  activeNavigationFlush=()=>status===SaveStatus.SAVED?Promise.resolve(true):persist();
  clearPageHideHandler();
  activePageHideHandler=()=>{if(status!==SaveStatus.SAVED)void persist();};
  ...
  $('#saveFailureHelp').addEventListener('click',(event)=>{if(event.target.closest('[data-action="retry-save"]'))void persist();});
  ...
  activeProductEditor=new ProductConfigurationEditor($('#productEditor'),{initialSnapshot:draft.product_configuration_snapshot,onSnapshot:(snapshot)=>{draft.product_configuration_snapshot=snapshot;markDirty();}});
  try{await activeProductEditor.mount();}catch(error){$('#productEditor').innerHTML=`<div class="notice error">商品Runtimeを読み込めませんでした: ${esc(error.message)}</div>`;}
...
async function renderRoute(){
  activeProductEditor?.destroy();activeProductEditor=null;const parts=location.pathname.split('/').filter(Boolean);
  try{
