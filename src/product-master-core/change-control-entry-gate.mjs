...
export function openGovernedChangeControl({
  proposal,approval,humanApprovalProvenance,reviewQueue,adjudicationStore,baseMaster
}={}){
  const errors=[];
  ...
  const humanApprovalGate={
    ...
    authority:{humanApprovalRequired:true,geminiApprovalAllowed:false,chatgptHumanImpersonationAllowed:false,changeControlOpen:true,productionMasterWriteAllowed:false,runtimeWriteAllowed:false}
  };
...
export function validateGovernedChangeControlEntry({
  approvedProposal,approval,humanApprovalProvenance,humanApprovalGate,reviewQueue,adjudicationStore,baseMaster
}={}){
  const errors=[];
...
export function applyGovernedApprovedProductMasterChangeProposal({
  ...
  const gate=validateGovernedChangeControlEntry({approvedProposal,approval,humanApprovalProvenance,humanApprovalGate,reviewQueue,adjudicationStore,baseMaster});
