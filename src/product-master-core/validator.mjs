...
export function validateProductMasterCore(master){
  ...
  for(const evidence of master?.evidence??[]){
    if(!evidence.title)push(errors,'EVIDENCE_TITLE_MISSING','Evidence title is required',evidence.id);
  ...
  if((master?.pending??[]).some((row)=>row.status==='OPEN'&&row.severity!=='BLOCKING'))push(warnings,'OPEN_NON_BLOCKING_PENDING','Open non-blocking PENDING exists');
