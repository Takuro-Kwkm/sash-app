...
export function validateGeminiTransportEnvelope(envelope,{expectedProductId=null,knownFields=CANONICAL_FIELD_NAMES,nodeIds=new Set()}={}){
  const errors=[];
  ...
  if(envelope.producer?.system!==GEMINI_PRODUCER_SYSTEM)errors.push(error('TRANSPORT_PRODUCER_INVALID',`producer.system must be ${GEMINI_PRODUCER_SYSTEM}`));
  ...
  for(const [index,row]of (envelope.candidates??[]).entries()){
    ...
    if(row.sourceSystem!==GEMINI_PRODUCER_SYSTEM)errors.push(error('TRANSPORT_CANDIDATE_SOURCE_SYSTEM_INVALID',`Candidate ${row.id??index} must use sourceSystem=${GEMINI_PRODUCER_SYSTEM}`,row.id));
