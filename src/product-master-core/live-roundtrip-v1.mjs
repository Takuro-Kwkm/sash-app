...
export function runApw430LiveEvidenceRoundTrip({
  ...
  result.report.gates.LIVE_EXTERNAL_INPUT=result.report.gates.EXTERNAL_INPUT;
  fs.writeFileSync(path.join(result.artifactDir,'report.json'),`${JSON.stringify(result.report,null,2)}\n`,'utf8');
  return result;
}
