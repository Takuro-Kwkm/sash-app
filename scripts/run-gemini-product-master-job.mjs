...
if(!jobFile)throw new Error('Usage: node scripts/run-gemini-product-master-job.mjs --job=<job.json> [--mock-response=file|--replay-response=file]');
...
if(!created.pass){console.log(JSON.stringify({pass:false,status:'JOB_INVALID',errors:created.errors},null,2));process.exitCode=2;}
  ...
  const result=await runGeminiProductMasterBridge(created.job,{evidenceInboxDir,changeControlDir,mockResponse,replayResponse});
  ...
  console.log(JSON.stringify({pass:result.pass,status:result.status,jobId:created.job.jobId,rawResponseSha256:result.rawResponseSha256,normalizedBatchId:result.normalizedBatchId??null,auditPath,canonicalWritePerformed:result.canonicalWritePerformed,runtimeWritePerformed:result.runtimeWritePerformed,productionWritePerformed:result.productionWritePerformed,errors:result.errors},null,2));
