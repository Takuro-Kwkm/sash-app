...
function runtimeSpecificSpecs(choice){
  const raw=choice?.runtimeValueRow?.metadata?.specific_spec;
  if(raw===undefined||raw===null||raw==='')return[];
  if(raw&&typeof raw==='object'&&!Array.isArray(raw)){
    if(Array.isArray(raw.$in))return raw.$in.flatMap(splitSpecificSpec);
    if(raw.$eq!==undefined)return splitSpecificSpec(raw.$eq);
  }
  return splitSpecificSpec(raw);
}
function choiceMatches(item,choice){
  const accepted=item.values??[];
  if(accepted.some((value)=>item.source_key==='specific_spec'?sourceSpecMatches(value,choice?.value):same(value,choice?.value)))return true;
  if(item.source_key!=='specific_spec')return false;
  const formalSpecs=runtimeSpecificSpecs(choice);
  return accepted.some((value)=>formalSpecs.some((spec)=>sourceSpecMatches(value,spec)));
}
function choiceEvidence(choice){return{value:choice?.value,formal_specific_specs:runtimeSpecificSpecs(choice)};}
...
function geometrySignature(rule){
  const {id,source_kind,window_id,source_judge_code,selector,...geometry}=rule;
  return stableJson(geometry);
}
...
function inertRuleForKey(seriesRules,rule,key){
  ...
  return new Set(siblings.map(geometrySignature)).size===1;
}
...
function desiredStatus(result,desired){
  ...
  for(const item of desired){
    ...
    for(const alias of item.aliases){
      if(!selected(result.selection,alias))continue;
      const field=visible.get(alias);
      const selectedChoice=enabled(field).find((choice)=>same(choice.value,result.selection[alias]));
      if((selectedChoice&&choiceMatches(item,selectedChoice))||(!selectedChoice&&(item.values??[]).some((value)=>same(result.selection[alias],value)))){
        satisfied={alias,selectedChoice};break;
      }
    }
    if(satisfied){states.push({...item,status:'SATISFIED',field:satisfied.alias,selected_value:result.selection[satisfied.alias],selected_choice:satisfied.selectedChoice?choiceEvidence(satisfied.selectedChoice):null});continue;}
    const candidateFields=item.aliases.filter((key)=>visible.has(key)).map((key)=>({key,field:visible.get(key),matches:enabled(visible.get(key)).filter((choice)=>choiceMatches(item,choice))}));
    const satisfiableVisible=candidateFields.filter((row)=>row.matches.length);
    if(satisfiableVisible.length){
      states.push({...item,status:'VISIBLE_SELECTABLE',candidate_fields:candidateFields.map((r)=>({key:r.key,value_count:enabled(r.field).length,matching_count:r.matches.length,matching_values:r.matches.map(choiceEvidence)}))});continue;
    }
    if(item.implicit_geometry_inert===true){
      states.push({...item,status:'IMPLICIT_GEOMETRY_INERT',candidate_fields:candidateFields.map((r)=>({key:r.key,value_count:enabled(r.field).length,matching_count:0})),proof:item.implicit_geometry_proof});continue;
    }
    states.push({...item,status:'NOT_YET_SELECTABLE',candidate_fields:candidateFields.map((r)=>({key:r.key,value_count:enabled(r.field).length,matching_count:0}))});
  }
  return states;
}
...
async function findReachableDirected(productId,windowId,desired){
  ...
  while(stack.length&&visited.size<MAX_STATES){
    ...
    const state=stack.pop();const inputKey=stableJson(state.selection);if(visited.has(inputKey))continue;visited.add(inputKey);
    ...
    for(const item of statuses.filter((x)=>x.status==='VISIBLE_SELECTABLE')){
      for(const alias of item.aliases){
        ...
        for(const choice of enabled(f).filter((candidate)=>choiceMatches(item,candidate))){
          desiredBranches.push({selection:f.dataType==='MULTI_ENUM'?branchMultiValue(canonical,alias,choice.value):branchValue(canonical,alias,choice.value),trace:[...state.trace,`${alias}=${choice.value} [selector:${item.source_key}:normalized]`]});
        }
      }
    ...
    const unsatisfied=statuses.filter((x)=>!['SATISFIED','IMPLICIT_GEOMETRY_INERT'].includes(x.status));
    ...
    if(mode&&customChoice&&!same(canonical.size_mode,'CUSTOM')){
      stack.push({selection:branchValue(canonical,'size_mode',customChoice.value),trace:[...state.trace,'size_mode=CUSTOM']});continue;
    }
    ...
    if(required.length){
      const f=required[0];
      const branches=enabled(f).map((choice)=>({selection:f.dataType==='MULTI_ENUM'?branchMultiValue(canonical,f.key,choice.value):branchValue(canonical,f.key,choice.value),trace:[...state.trace,`${f.key}=${choice.value} [required-directed]`]}));
      for(const branch of branches.reverse())stack.push(branch);continue;
    }
    ...
    if(optional.length){
      const f=optional[0];
      const branches=enabled(f).map((choice)=>({selection:f.dataType==='MULTI_ENUM'?branchMultiValue(canonical,f.key,choice.value):branchValue(canonical,f.key,choice.value),trace:[...state.trace,`${f.key}=${choice.value} [optional-directed]`]}));
      for(const branch of branches.reverse())stack.push(branch);continue;
    }
...
const ruleBySeriesId=new Map();const seriesRulesByName=new Map();
...
for(const row of contextMap.values()){
  idx++;const result=await findReachableDirected(row.product_id,row.window_id,row.exposed_selector);
...
const report={exact_head_sha:HEAD_SHA,task_classification:'NON-PRODUCT-MASTER',product_master_mutation:0,model_version:'CUSTOM_SELECTOR_UI_REACHABILITY_DIRECTED_V3_NORMALIZED',normalization_model:{specific_spec_pipe_semantics:'ONE_OF',specific_spec_runtime_metadata_bridge:'runtimeValueRow.metadata.specific_spec',implicit_selector_policy:'GEOMETRY_INERT_ONLY',search:'DIRECTED_DFS'},source_shape_digest:shape.evidence_digest,source_rule_count:sourceRuleCount,covered_rule_count:coveredRuleIds.size,unique_context_count:contexts.length,reachable_context_count:contexts.length-unreachable.length,unreachable_context_count:unreachable.length,implicit_geometry_inert_selector_count:implicitProofCount,total_visited_state_count:contexts.reduce((n,r)=>n+r.visited_state_count,0),total_resolver_call_count:contexts.reduce((n,r)=>n+r.resolver_call_count,0),contexts,status:unreachable.length?'BLOCKED_UNREACHABLE_CUSTOM_SELECTOR_CONTEXT':'PASS'};
report.evidence_digest=hash({head:HEAD_SHA,source:shape.evidence_digest,model:report.model_version,contexts:contexts.map((r)=>({series:r.series,window:r.window_id,exposed:r.exposed_selector,rules:r.rule_ids,hidden:r.hidden_constraints,reachable:r.reachable,selection:r.selection_prefix??null,trace:r.trace??null,reason:r.reason??null}))});
...
console.log(`CUSTOM_SELECTOR_REACHABILITY_GATE=${unreachable.length?'BLOCKED':'PASS'} contexts=${contexts.length} reachable=${report.reachable_context_count} rules=${report.covered_rule_count}/${report.source_rule_count} implicit_geometry_inert=${implicitProofCount}`);
