...
const catalogVersion = "V4.3 RECOVERY + WINDOW-MASTERS-WAVE1";
...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/catalog/products") {
    return json(res, 200, catalog.products);
  }
  if (url.pathname === "/api/catalog/fields") {
    const productId = url.searchParams.get("productId");
    return json(res, 200, catalog.specificationDefinitions.filter((row) => !productId || row.productId === productId));
  }
  if (url.pathname === "/api/catalog/allowed-values") {
    const productId = url.searchParams.get("productId");
    const key = url.searchParams.get("key");
    return json(res, 200, catalog.allowedValues.filter((row) => (!productId || row.productId === productId) && (!key || row.specificationKey === key)));
  }
