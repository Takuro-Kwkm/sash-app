...
const SOURCE_RUN='35997958795';
const SOURCE_HEAD='56f02579a21c9ce9a4b52797b98f3f422b0f07c1';
const SOURCE_ART=`uchirimo-heavy-recovery-analysis-${SOURCE_HEAD}`;
const SOURCE_ART_DIGEST='sha256:0fa2cc7d11ceedbff923a9325e0379776f9ed8ae3930a6b40e5e7fffba024017';
const SOURCE_ANALYSIS_BLOB='0dfa19a6557057f566a30bd5e3babe2f94ce6871';
...
const TIMEOUT=Number(process.env.UCHIRIMO_CONSTRAINT_DEPTH4_MICRO_TIMEOUT_MS??60000);
const REPO=String(process.env.GITHUB_REPOSITORY??'Takuro-Kwkm/sash-app');
const head=currentExactHead();
if(!Number.isFinite(TIMEOUT)||TIMEOUT<60000)throw new Error('CONSTRAINT_DEPTH4_TIMEOUT_INVALID');
...
async function applyConstraints(prefix,constraints){
  ...
  if(!prefixPreserved(result,prefix))throw new Error('CONSTRAINT_DEPTH4_PREFIX_REJECTED');
  for(const constraint of constraints){
    ...
    if(!survives(result,constraint.field_key,constraint.decision))throw new Error('CONSTRAINT_DEPTH4_SOURCE_CONSTRAINT_NOT_PRESERVED:'+constraint.field_key);
    if(!prefixPreserved(result,prefix))throw new Error('CONSTRAINT_DEPTH4_SOURCE_CONSTRAINT_CLEARED_PREFIX:'+constraint.field_key);
  }
  return result;
}
...
function constraintRow(parent,index,constraints){
  ...
  return {
    ...
    partition_key:`${parent.parent_child_id}|constraint-depth4-representative-${index}`,
...
function executeConstraint(parent,index,constraints){
  ...
  const id=`constraint-depth4-representative-${String(index).padStart(2,'0')}`;
  const dir=`${OUT}/constraint-depth4-representative/case-${String(index).padStart(2,'0')}`;
...
const sourceBinding=read(`${src}/constraint-depth3-all-branch-source-binding.json`);
const sourceModel=read(`${src}/explicit-decision-constraint-depth3-partition-model-proof.json`);
const sourceRepresentative=read(`${src}/explicit-decision-constraint-depth3-representative-micro-calibration.json`);
const depth3Calibration=read(`${src}/explicit-decision-constraint-depth3-all-branch-calibration.json`);
const sourceDecision=read(`${src}/constraint-depth3-all-branch-decision.json`);
...
for(const artifact of [heavy,recursive,sourceBinding,sourceModel,sourceRepresentative,depth3Calibration,sourceDecision,sourceDepth11Plan,sourceDepth11Proof,sourceDepth11Micro])if(artifact.exact_head!==SOURCE_HEAD)throw new Error('SOURCE_HEAD_MISMATCH');
...
if(sourceDecision.REQUESTED_DIFF_COVERAGE!=='PASS'||sourceDecision.UNREQUESTED_DIFF_COUNT!==0||sourceDecision.next_constraint_action!=='CONSTRAINT_DEPTH4_REQUIRED_FOR_SLOW_DEPTH3_BRANCHES')throw new Error('SOURCE_DEPTH3_DECISION_INVALID');
...
const binding={schema_version:'1.0.0',artifact_type:'UCHIRIMO_CONSTRAINT_DEPTH4_REPRESENTATIVE_SOURCE_BINDING',exact_head:head,source_exact_head:SOURCE_HEAD,source_run_id:Number(SOURCE_RUN),source_artifact_name:SOURCE_ART,source_artifact_digest:SOURCE_ART_DIGEST,changed_files_since_source:changed,source_constraint_depth3_expected_branch_count:depth3Calibration.expected_branch_count,source_constraint_depth3_pass_count:depth3Calibration.pass_count,source_constraint_depth3_slow_count:depth3Calibration.needs_further_partitioning_count,source_constraint_depth3_invalid_count:depth3Calibration.calibration_invalid_count,source_depth3_all_branch_measurement_reexecuted:false,source_depth3_representative_measurement_reexecuted:false,source_depth2_branch_measurement_reexecuted:false,source_depth11_measurement_reexecuted:false,current_runtime_manifest_sha256:runtime.sourcePackageIntegrity.actual,batch_runner_unchanged:true,full_selector_runner_unchanged:true,status:'PASS'};
writeJson(`${OUT}/constraint-depth4-representative-source-binding.json`,binding);
writeJson(`${OUT}/heavy-partition-analysis.json`,{...heavy,exact_head:head,evidence_origin:'CURRENT_HEAD_CONSTRAINT_DEPTH4_BINDING',source_bound_exact_head:SOURCE_HEAD,current_head_binding_sha256:hash(binding),source_pass_evidence_reused_as_current_head:false});
writeJson(`${OUT}/recursive-partition-plan.json`,{...recursive,exact_head:head,evidence_origin:'CURRENT_HEAD_CONSTRAINT_DEPTH4_BINDING',source_bound_exact_head:SOURCE_HEAD,current_head_binding_sha256:hash(binding),source_pass_evidence_reused_as_current_head:false});
...
writeJson(`${OUT}/explicit-decision-constraint-depth3-partition-model-proof.json`,{...sourceModel,exact_head:head,measurement_reexecuted:false,evidence_origin:'BOUND_SOURCE_DIAGNOSTIC_NOT_CURRENT_EXECUTION'});
writeJson(`${OUT}/explicit-decision-constraint-depth3-representative-micro-calibration.json`,{...sourceRepresentative,exact_head:head,measurement_reexecuted:false,evidence_origin:'BOUND_SOURCE_DIAGNOSTIC_NOT_CURRENT_EXECUTION'});
writeJson(`${OUT}/explicit-decision-constraint-depth3-all-branch-calibration.json`,{...depth3Calibration,exact_head:head,measurement_reexecuted:false,evidence_origin:'BOUND_SOURCE_DIAGNOSTIC_NOT_CURRENT_EXECUTION'});

const depth3ById=new Map((sourceModel.children??[]).map(child=>[child.child_id,child]));
const slowRows=(depth3Calibration.results??[]).filter(row=>row.status==='NEEDS_FURTHER_PARTITIONING');
if(slowRows.length!==2)throw new Error('CONSTRAINT_DEPTH4_SOURCE_SLOW_COUNT_INVALID:'+slowRows.length);
const parents=[];
const children=[];
for(const source of slowRows){
  const depth3Child=depth3ById.get(source.child_id);
  if(!depth3Child)throw new Error('CONSTRAINT_DEPTH4_SOURCE_CHILD_MISSING:'+source.child_id);
  const prefix=stable(depth3Child.selector_prefix??{});
  const sourceConstraints=stable(depth3Child.constraints??[]);
  if(sourceConstraints.length!==3||sourceConstraints[1]?.field_key!=='extension_frame_type'||sourceConstraints[1]?.decision?.value!=='fukashi_60'||sourceConstraints[2]?.field_key!=='extension_frame_reinforcement'||sourceConstraints[2]?.decision?.value!=='reinforcement_bracket')throw new Error('CONSTRAINT_DEPTH4_SOURCE_CONSTRAINT_SHAPE_INVALID:'+source.child_id);
  ...
  for(const field of candidates){
    ...
    for(const decision of domain){
      ...
      const fourthOk=survives(child,field.key,decision);
      const prefixOk=prefixPreserved(child,prefix);
      if(constraintsOk&&fourthOk&&prefixOk)reachable.push({decision:stable(decision),decision_key:dkey(decision),selection_fingerprint:hash(stable(child.selection??{}))});
      else rejected.push({decision:stable(decision),reason:!constraintsOk?'SOURCE_CONSTRAINT_CLEARED':!fourthOk?'FOURTH_DECISION_REJECTED_OR_CLEARED':'PREFIX_CLEARED'});
    }
  ...
  if(!chosen)throw new Error('CONSTRAINT_DEPTH4_NO_SAFE_DECISION_AXIS:'+source.child_id);
  const parent={parent_child_id:source.child_id,parent_lane_id:source.parent_lane_id,product_node:source.product_node,selector_prefix:prefix,source_constraints:sourceConstraints,fourth_constraint_field:chosen.field_key,fourth_constraint_data_type:chosen.data_type,candidate_proofs:candidateProofs,reachable_branch_count:chosen.reachable_branch_count,rejected_branch_count:chosen.rejected_branch_count,status:'PASS'};
  parents.push(parent);
  for(const reachable of chosen.reachable_children){
    ...
    children.push({child_id:`UHC-C4-${hash([source.child_id,chosen.field_key,reachable.decision]).slice(0,20)}`,parent_child_id:source.child_id,parent_lane_id:source.parent_lane_id,product_node:source.product_node,selector_prefix:prefix,constraints,fourth_constraint_field:chosen.field_key,fourth_decision:reachable.decision,selection_fingerprint:reachable.selection_fingerprint,status:'PLANNED_UNVERIFIED'});
  }
}
...
const proof={schema_version:'1.0.0',artifact_type:'UCHIRIMO_EXPLICIT_DECISION_CONSTRAINT_DEPTH4_PARTITION_MODEL_PROOF',exact_head:head,source_exact_head:SOURCE_HEAD,source_run_id:Number(SOURCE_RUN),parent_partition_count:parents.length,child_partition_count:children.length,PARTITION_OVERLAP_COUNT:overlap,PARTITION_GAP_COUNT:gap,parents,children,coverage_preservation_status:proofPass?'PASS':'FAIL',constraint_depth4_full_execution_authorized:false,full_coverage_authorized:false,status:proofPass?'PASS':'BLOCKED'};
writeJson(`${OUT}/explicit-decision-constraint-depth4-partition-model-proof.json`,proof);
if(!proofPass)throw new Error('CONSTRAINT_DEPTH4_MODEL_PROOF_FAIL');

const results=[];
for(const [index,parent] of parents.entries()){
  const representative=children.filter(row=>row.parent_child_id===parent.parent_child_id).sort((a,b)=>dkey(a.fourth_decision).localeCompare(dkey(b.fourth_decision)))[0];
  if(!representative)throw new Error('CONSTRAINT_DEPTH4_REPRESENTATIVE_MISSING:'+parent.parent_child_id);
  ...
  results.push({index,parent_child_id:parent.parent_child_id,parent_lane_id:parent.parent_lane_id,product_node:parent.product_node,source_constraints:parent.source_constraints,fourth_constraint_field:parent.fourth_constraint_field,fourth_decision:representative.fourth_decision,constraints:representative.constraints,...measured,status});
  console.log(`CONSTRAINT_DEPTH4_REPRESENTATIVE parent=${parent.parent_child_id} field=${parent.fourth_constraint_field} decision=${dkey(representative.fourth_decision)} status=${status} outcome=${measured.outcome} states=${measured.visited_state_count??null} terminals=${measured.terminal_context_count??null}`);
}
...
const verified=invalid.length===0&&results.length===2&&results.every(row=>row.constraint_start_valid===true&&row.constraint_count===4&&(row.status==='PASS'||row.constraint_progress_valid===true));
const micro={schema_version:'1.0.0',artifact_type:'UCHIRIMO_EXPLICIT_DECISION_CONSTRAINT_DEPTH4_REPRESENTATIVE_MICRO_CALIBRATION',exact_head:head,source_exact_head:SOURCE_HEAD,source_run_id:Number(SOURCE_RUN),representative_count:results.length,child_timeout_ms:TIMEOUT,pass_count:passed.length,needs_further_partitioning_count:deeper.length,calibration_invalid_count:invalid.length,constraint_depth4_representative_verified:verified,constraint_depth4_full_execution_authorized:false,full_coverage_authorized:false,source_depth3_all_branch_measurement_reexecuted:false,results,status:invalid.length||!verified?'BLOCKED':'MEASURED_CALIBRATION_ONLY'};
writeJson(`${OUT}/explicit-decision-constraint-depth4-representative-micro-calibration.json`,micro);

const nextConstraintAction=!verified?'CONSTRAINT_DEPTH4_REPRESENTATIVE_BLOCKED':deeper.length?'CONSTRAINT_DEPTH5_REQUIRED_FOR_SLOW_DEPTH4_BRANCHES':'CONSTRAINT_DEPTH4_REPRESENTATIVE_FAST_PATH_PROMISING';
const decision={schema_version:'1.0.0',artifact_type:'UCHIRIMO_CONSTRAINT_DEPTH4_REPRESENTATIVE_DECISION',exact_head:head,source_exact_head:SOURCE_HEAD,source_run_id:Number(SOURCE_RUN),source_constraint_depth3_all_branch_runner_verified:depth3Calibration.constraint_depth3_all_branch_runner_verified,source_constraint_depth3_pass_count:depth3Calibration.pass_count,source_constraint_depth3_slow_count:depth3Calibration.needs_further_partitioning_count,source_constraint_depth3_invalid_count:depth3Calibration.calibration_invalid_count,constraint_depth4_model_status:proof.status,constraint_depth4_parent_count:parents.length,constraint_depth4_child_count:children.length,constraint_depth4_representative_pass_count:passed.length,constraint_depth4_representative_needs_further_partitioning_count:deeper.length,constraint_depth4_representative_calibration_invalid_count:invalid.length,constraint_depth4_representative_verified:verified,next_constraint_action:nextConstraintAction,next_safe_lane_action:sourceDecision.next_safe_lane_action,source_depth3_all_branch_measurement_reexecuted:false,source_depth3_representative_measurement_reexecuted:false,source_depth2_branch_measurement_reexecuted:false,source_depth11_measurement_reexecuted:false,constraint_depth4_full_execution_authorized:false,full_depth11_execution_authorized:false,full_coverage_authorized:false,REQUESTED_CHANGE_SCOPE:'UCHIRIMO_HEAVY_RECOVERY_CONSTRAINT_DEPTH4_MODEL_AND_TWO_REPRESENTATIVE_MICROS_ONLY',REQUESTED_DIFF_COVERAGE:'PASS',UNREQUESTED_DIFF_COUNT:0,UCHIRIMO_FULL_COVERAGE_QA_GATE:'BLOCKED',UCHIRIMO_QA_STATUS:'UNVERIFIED',APP_INTEGRATION_READY:false,RELEASE_INPUT_GATE:'BLOCKED',status:invalid.length||!verified?'BLOCKED':'DIAGNOSTIC_COMPLETE'};
writeJson(`${OUT}/constraint-depth4-representative-decision.json`,decision);

console.log(`CONSTRAINT_DEPTH4_MODEL_STATUS=${proof.status}`);
console.log(`CONSTRAINT_DEPTH4_PARENT_COUNT=${parents.length}`);
console.log(`CONSTRAINT_DEPTH4_CHILD_COUNT=${children.length}`);
console.log(`CONSTRAINT_DEPTH4_REPRESENTATIVE_PASS_COUNT=${passed.length}/${results.length}`);
console.log(`CONSTRAINT_DEPTH4_REPRESENTATIVE_VERIFIED=${verified?'TRUE':'FALSE'}`);
console.log('SOURCE_DEPTH3_ALL_BRANCH_MEASUREMENT_REEXECUTED=FALSE');
console.log('SOURCE_DEPTH3_REPRESENTATIVE_MEASUREMENT_REEXECUTED=FALSE');
...
console.log('CONSTRAINT_DEPTH4_FULL_EXECUTION_AUTHORIZED=FALSE');
...
if(invalid.length||!verified)throw new Error('CONSTRAINT_DEPTH4_REPRESENTATIVE_INVALID');
