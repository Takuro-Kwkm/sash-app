...
const executionCoverageRoots=parentClosureMap.map(row=>{
  const isBath=row.unverified_reason==='BATH_DEPTH1_REPRESENTATIVE_PASS_NOT_PARENT_COMPLETE';
  const requiredIds=(isBath?row.depth1_child_ids:row.depth2_child_ids).slice().sort();
  return {original_parent_partition_id:row.original_parent_partition_id,product_node:row.product_node,window_id:row.window_id,execution_stage:isBath?'STAGE1_BATH_DEPTH1_EXISTING_PARTITIONS':'STAGE2_NONBATH_DEPTH2_EXISTING_PARTITIONS',required_existing_partition_count:requiredIds.length,required_existing_partition_ids:requiredIds,representative_deeper_evidence_present:row.depth5_representative_parent_count>0,representative_deeper_evidence_reusable_as_root_closure:false,closure_requirement:'ALL_REQUIRED_EXISTING_PARTITIONS_TERMINAL_PASS',failure_recovery_rule:'DEEPEN_ONLY_EXACT_FAILED_OR_TIMED_OUT_EXISTING_PARTITIONS_AFTER_CURRENT_HEAD_EXECUTION'};
});
const stage1Coverage=executionCoverageRoots.filter(x=>x.execution_stage==='STAGE1_BATH_DEPTH1_EXISTING_PARTITIONS');
const stage2Coverage=executionCoverageRoots.filter(x=>x.execution_stage==='STAGE2_NONBATH_DEPTH2_EXISTING_PARTITIONS');
const stage1UnitCount=stage1Coverage.reduce((n,x)=>n+x.required_existing_partition_count,0);
const stage2UnitCount=stage2Coverage.reduce((n,x)=>n+x.required_existing_partition_count,0);
const executionUnitIds=executionCoverageRoots.flatMap(x=>x.required_existing_partition_ids);
const runnerBatchItemLimit=2;
const stage1LogicalBatchCount=Math.ceil(stage1UnitCount/runnerBatchItemLimit);
const stage2LogicalBatchCount=Math.ceil(stage2UnitCount/runnerBatchItemLimit);
const stage2RepresentativeDepth5RootCount=stage2Coverage.filter(x=>x.representative_deeper_evidence_present).length;
if(executionCoverageRoots.length!==514||stage1Coverage.length!==38||stage2Coverage.length!==476||stage1UnitCount!==76||stage2UnitCount!==7364||executionUnitIds.length!==7440||new Set(executionUnitIds).size!==7440||stage1LogicalBatchCount!==38||stage2LogicalBatchCount!==3682||stage2RepresentativeDepth5RootCount!==10||executionCoverageRoots.some(x=>x.required_existing_partition_count<1))throw new Error('ORIGINAL514_EXISTING_PARTITION_EXECUTION_COVERAGE_INVALID');

const runnerBaseKeys=new Set(['room_specification','window_type','sash_configuration','size_class','glass_family']);
const stage1RunnerItems=bathDepth1.slice().sort((a,b)=>String(a.PARENT_PARTITION_ID).localeCompare(String(b.PARENT_PARTITION_ID))||String(a.PARTITION_ID).localeCompare(String(b.PARTITION_ID))).map((child,shard)=>{
  const prefix=stable(child.SELECTOR_PREFIX??{});
  for(const key of ['room_specification','window_type','sash_configuration','size_class','glass_family'])if(prefix[key]==null||prefix[key]==='')throw new Error('STAGE1_RUNNER_REQUIRED_AXIS_MISSING:'+child.PARTITION_ID+':'+key);
  const partitionSeed=Object.fromEntries(Object.entries(prefix).filter(([key])=>!runnerBaseKeys.has(key)));
  return {shard,node_id:String(child.PRODUCT_NODE),room_specification:String(prefix.room_specification),window_type:String(prefix.window_type),sash_configuration:String(prefix.sash_configuration),size_class:String(prefix.size_class),glass_family:String(prefix.glass_family),partition_key:String(child.PARTITION_ID),partition_seed_json:JSON.stringify(stable(partitionSeed)),decision_constraints_json:'[]',original_parent_partition_id:String(child.PARENT_PARTITION_ID),source_partition_depth:1};
});
const stage1RunnerBatches=[];
for(let index=0;index<stage1RunnerItems.length;index+=runnerBatchItemLimit)stage1RunnerBatches.push({batch_id:`stage1-bath-depth1-${String(index/runnerBatchItemLimit).padStart(2,'0')}`,batch_index:index/runnerBatchItemLimit,items:stage1RunnerItems.slice(index,index+runnerBatchItemLimit)});
const stage1RunnerRootCounts=stage1RunnerItems.reduce((m,x)=>(m[x.original_parent_partition_id]=(m[x.original_parent_partition_id]??0)+1,m),{});
if(stage1RunnerItems.length!==76||stage1RunnerBatches.length!==38||stage1RunnerBatches.some(x=>x.items.length!==2)||stage1RunnerItems.some((x,index)=>x.shard!==index)||new Set(stage1RunnerItems.map(x=>x.partition_key)).size!==76||Object.keys(stage1RunnerRootCounts).length!==38||Object.values(stage1RunnerRootCounts).some(x=>x!==2))throw new Error('STAGE1_BATH_RUNNER_INPUT_INVALID');
...
const binding={source_exact_head:SH,source_run_id:Number(SR),source_artifact_identity:SA,source_artifact_sha256:SD,current_exact_head:head,changed_paths:changed,dependency_fingerprint:dependencyFingerprint,impact_decision:'STAGE1_BATH_DEPTH1_EXISTING_PARTITION_RUNNER_INPUT_ONLY_SOURCE_MEASUREMENTS_BOUND_AND_REVALIDATED',measurement_reexecuted:false};
...
writeJson(`${OUT}/explicit-constraint-original514-existing-partition-execution-coverage-plan.json`,{schema_version:'1.0.0',artifact_type:'UCHIRIMO_ORIGINAL514_EXISTING_PARTITION_EXECUTION_COVERAGE_PLAN',exact_head:head,current_runtime_manifest_sha256:runtimeHash,current_head_evidence_binding:binding,coverage_strategy:'STAGED_EXISTING_PARTITION_EXECUTION_NO_NEW_PARTITIONS',original_heavy_parent_count:514,coverage_plan_root_count:executionCoverageRoots.length,runner_batch_item_limit:runnerBatchItemLimit,total_existing_partition_execution_unit_count:executionUnitIds.length,stage1:{stage:'STAGE1_BATH_DEPTH1_EXISTING_PARTITIONS',root_count:stage1Coverage.length,partition_depth:1,existing_partition_execution_unit_count:stage1UnitCount,logical_batch_count:stage1LogicalBatchCount,runner_input_ready:true,execution_authorized:false,coverage_roots:stage1Coverage},stage2:{stage:'STAGE2_NONBATH_DEPTH2_EXISTING_PARTITIONS',root_count:stage2Coverage.length,partition_depth:2,existing_partition_execution_unit_count:stage2UnitCount,logical_batch_count:stage2LogicalBatchCount,representative_depth5_root_count:stage2RepresentativeDepth5RootCount,representative_depth5_evidence_reusable_as_root_closure:false,execution_authorized:false,coverage_roots:stage2Coverage},all_514_roots_have_existing_partition_execution_coverage:true,coverage_plan_gap_count:0,new_partition_authorized:false,deeper_split_authorized:false,execution_performed:false,status:'PLAN_READY'});
writeJson(`${OUT}/explicit-constraint-stage1-bath-depth1-existing-partition-runner-input.json`,{schema_version:'1.0.0',artifact_type:'UCHIRIMO_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_RUNNER_INPUT',exact_head:head,current_runtime_manifest_sha256:runtimeHash,current_head_evidence_binding:binding,runner_path:B,runner_blob_sha:BB,full_selector_proof_path:F,full_selector_proof_blob_sha:FB,execution_stage:'STAGE1_BATH_DEPTH1_EXISTING_PARTITIONS',root_count:38,expected_shards:76,item_count:stage1RunnerItems.length,batch_item_limit:runnerBatchItemLimit,batch_count:stage1RunnerBatches.length,child_timeout_ms:1080000,runner_items:stage1RunnerItems,runner_batches:stage1RunnerBatches,all_38_roots_have_two_existing_depth1_items:true,partition_key_unique_count:76,shard_range:{min:0,max:75,contiguous:true},new_partition_authorized:false,deeper_split_authorized:false,execution_authorized:false,execution_performed:false,status:'INPUT_READY'});
writeJson(`${OUT}/explicit-constraint-stage1-bath-depth1-existing-partition-runner-input-decision.json`,{schema_version:'1.0.0',artifact_type:'UCHIRIMO_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_RUNNER_INPUT_DECISION',exact_head:head,stage1_runner_input_ready:true,stage1_bath_root_count:38,stage1_existing_depth1_partition_count:76,stage1_logical_batch_count:38,stage1_expected_shards:76,stage1_execution_authorized:false,next_recovery_action:'AUTHORIZE_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_EXECUTION_AFTER_CURRENT_HEAD_INPUT_PROOF',new_partition_authorized:false,deeper_split_authorized:false,full_constraint_execution_authorized:false,full_coverage_authorized:false,REQUESTED_CHANGE_SCOPE:'UCHIRIMO_HEAVY_RECOVERY_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_RUNNER_INPUT',REQUESTED_DIFF_COVERAGE:'PASS',UNREQUESTED_DIFF_COUNT:0,UCHIRIMO_FULL_COVERAGE_QA_GATE:'BLOCKED',UCHIRIMO_QA_STATUS:'UNVERIFIED',APP_INTEGRATION_READY:false,RELEASE_INPUT_GATE:'BLOCKED',status:'INPUT_READY'});
writeJson(`${OUT}/explicit-constraint-original514-existing-partition-execution-coverage-decision.json`,{schema_version:'1.0.0',artifact_type:'UCHIRIMO_ORIGINAL514_EXISTING_PARTITION_EXECUTION_COVERAGE_DECISION',exact_head:head,execution_coverage_plan_ready:true,stage1_runner_input_ready:true,original_heavy_parent_count:514,coverage_plan_root_count:514,coverage_plan_gap_count:0,stage1_bath_root_count:38,stage1_existing_depth1_partition_count:76,stage1_logical_batch_count:38,stage2_nonbath_root_count:476,stage2_existing_depth2_partition_count:7364,stage2_logical_batch_count:3682,stage2_representative_depth5_root_count:10,next_recovery_action:'AUTHORIZE_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_EXECUTION_AFTER_CURRENT_HEAD_INPUT_PROOF',stage1_execution_authorized:false,stage2_execution_authorized:false,new_partition_authorized:false,deeper_split_authorized:false,full_constraint_execution_authorized:false,full_coverage_authorized:false,REQUESTED_CHANGE_SCOPE:'UCHIRIMO_HEAVY_RECOVERY_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_RUNNER_INPUT',REQUESTED_DIFF_COVERAGE:'PASS',UNREQUESTED_DIFF_COUNT:0,UCHIRIMO_FULL_COVERAGE_QA_GATE:'BLOCKED',UCHIRIMO_QA_STATUS:'UNVERIFIED',APP_INTEGRATION_READY:false,RELEASE_INPUT_GATE:'BLOCKED',status:'INPUT_READY'});
writeJson(`${OUT}/explicit-constraint-original514-unverified-branch-decision.json`,{schema_version:'1.0.0',artifact_type:'UCHIRIMO_ORIGINAL514_UNVERIFIED_BRANCH_DECISION',exact_head:head,original_heavy_parent_count:514,complete_514_parent_mapping_present:true,proven_closed_original_parent_count:0,unverified_original_parent_count:514,bath_unverified_parent_count:38,depth5_representative_only_parent_count:10,depth2_planned_not_full_executed_parent_count:466,execution_coverage_plan_ready:true,stage1_runner_input_ready:true,next_recovery_action:'AUTHORIZE_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_EXECUTION_AFTER_CURRENT_HEAD_INPUT_PROOF',new_partition_authorized:false,full_constraint_execution_authorized:false,full_coverage_authorized:false,REQUESTED_CHANGE_SCOPE:'UCHIRIMO_HEAVY_RECOVERY_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_RUNNER_INPUT',REQUESTED_DIFF_COVERAGE:'PASS',UNREQUESTED_DIFF_COUNT:0,UCHIRIMO_FULL_COVERAGE_QA_GATE:'BLOCKED',UCHIRIMO_QA_STATUS:'UNVERIFIED',APP_INTEGRATION_READY:false,RELEASE_INPUT_GATE:'BLOCKED',status:'BLOCKED'});
writeJson(`${OUT}/explicit-constraint-frame-installation-mode21-to-original514-closure-proof.json`,{schema_version:'1.0.0',artifact_type:'UCHIRIMO_FRAME_INSTALLATION_MODE21_TO_ORIGINAL514_CLOSURE_PROOF',exact_head:head,current_runtime_manifest_sha256:runtimeHash,current_head_evidence_binding:binding,original_heavy_parent_count:514,initial_partition_child_count:2106,initial_partition_coverage_preservation_status:'PASS',frame_installation_mode_closed_child_count:21,frame_installation_mode_closure_status:'PASS',frame_model_lane_count:7,depth1_representative_calibrated_child_count:11,depth1_representative_pass_count:1,depth1_full_child_execution_authorized:false,bath_original_heavy_parent_count:38,bath_depth1_child_count:76,bath_representative_pass_child_count:1,bath_excluded_from_depth2:true,depth2_parent_count:2030,depth2_child_count:7364,depth2_full_execution_authorized:false,depth5_representative_parent_count:10,depth5_representative_child_count:33,depth5_diagnostic_scope:depth5Plan.diagnostic_scope,complete_514_parent_to_terminal_closure_mapping_present:true,proven_closed_original_parent_count:0,unverified_original_parent_count:514,execution_coverage_plan_ready:true,stage1_runner_input_ready:true,all_514_original_heavy_partitions_covered:false,blockers,execution_performed:false,status:'BLOCKED'});
writeJson(`${OUT}/explicit-constraint-original514-closure-decision.json`,{schema_version:'1.0.0',artifact_type:'UCHIRIMO_ORIGINAL514_CLOSURE_DECISION',exact_head:head,frame_installation_mode_21_closed:true,original_heavy_parent_count:514,complete_514_parent_mapping_present:true,proven_closed_original_parent_count:0,unverified_original_parent_count:514,execution_coverage_plan_ready:true,stage1_runner_input_ready:true,all_514_original_heavy_partitions_covered:false,closure_gap_count:blockers.length,next_recovery_action:'AUTHORIZE_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_EXECUTION_AFTER_CURRENT_HEAD_INPUT_PROOF',frame_reexecution_authorized:false,new_partition_authorized:false,full_constraint_execution_authorized:false,full_coverage_authorized:false,REQUESTED_CHANGE_SCOPE:'UCHIRIMO_HEAVY_RECOVERY_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_RUNNER_INPUT',REQUESTED_DIFF_COVERAGE:'PASS',UNREQUESTED_DIFF_COUNT:0,UCHIRIMO_FULL_COVERAGE_QA_GATE:'BLOCKED',UCHIRIMO_QA_STATUS:'UNVERIFIED',APP_INTEGRATION_READY:false,RELEASE_INPUT_GATE:'BLOCKED',status:'BLOCKED'});
...
console.log('EXISTING_PARTITION_EXECUTION_COVERAGE_PLAN=READY');
console.log('STAGE1_RUNNER_INPUT=READY');
console.log('STAGE1_BATH_ROOT_COUNT=38');
console.log('STAGE1_EXISTING_DEPTH1_PARTITION_COUNT=76');
console.log('STAGE1_LOGICAL_BATCH_COUNT=38');
console.log('STAGE1_EXPECTED_SHARDS=76');
console.log('STAGE1_EXECUTION_AUTHORIZED=FALSE');
console.log('ORIGINAL_514_CLOSURE=BLOCKED');
console.log('NEXT_RECOVERY_ACTION=AUTHORIZE_STAGE1_BATH_DEPTH1_EXISTING_PARTITION_EXECUTION_AFTER_CURRENT_HEAD_INPUT_PROOF');
