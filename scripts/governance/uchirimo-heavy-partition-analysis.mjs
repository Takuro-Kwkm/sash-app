...
const SR='36089961893';
const SH='ff5a24ac75966d1298e1e018ad41f147438e1985';
const SA=`uchirimo-heavy-recovery-analysis-${SH}`;
const SD='sha256:4dc580cc97d70703f6e2a8ebf8ad0895d9525b68ce6d79a508eacd98031b6157';
const SB='0a485db05ba678c42c988d33ae04a66194c80ef4';
...
async function applyConstraints(prefix,constraints){
  ...
  if(!prefixPreserved(result,prefix))throw new Error('WOOD19_PREFIX_REJECTED');
  for(const constraint of constraints){
    ...
    if(!survives(result,constraint.field_key,constraint.decision))throw new Error('WOOD19_SOURCE_CONSTRAINT_NOT_PRESERVED:'+constraint.field_key);
    if(!prefixPreserved(result,prefix))throw new Error('WOOD19_SOURCE_CONSTRAINT_CLEARED_PREFIX:'+constraint.field_key);
  }
  return result;
}
...
meta(SR,SA,SD);
const t=String(process.env.RUNNER_TEMP??'/tmp'),s=`${t}/uwood19-surface-${SR}`;
dl(SR,SA,s);
...
const exec38=rd(`${s}/explicit-constraint-remaining38-construction-execution.json`);
const dec38=rd(`${s}/explicit-constraint-remaining38-construction-decision.json`);
const constructionModel=rd(`${s}/explicit-constraint-slow19-bracket-construction-partition-model-proof.json`);
if(exec38.exact_head!==SH||exec38.executed_child_count!==38||exec38.pass_count!==19||exec38.nonempty_pass_count!==19||exec38.empty_pass_count!==0||exec38.needs_further_partitioning_count!==19||exec38.execution_invalid_count!==0||exec38.remaining38_execution_verified!==true||exec38.status!=='MEASURED_PARTIAL')throw new Error('REMAINING38_SOURCE_INVALID');
if(sj(exec38.slow_split_values)!==sj(['wood'])||exec38.unresolved_construction_child_count!==19||exec38.all_57_construction_children_covered!==false)throw new Error('REMAINING38_SLOW_SET_INVALID');
if(dec38.next_recovery_action!=='PARTITION_ONLY_SLOW_REMAINING_CONSTRUCTION_CHILDREN'||dec38.unresolved_construction_child_count!==19||dec38.status!=='PARTIAL_CLOSURE'||dec38.remaining_38_reexecution_authorized!==false||dec38.representative_reexecution_authorized!==false||dec38.full_57_reexecution_authorized!==false)throw new Error('REMAINING38_DECISION_INVALID');
if(constructionModel.status!=='PASS'||constructionModel.parent_count!==19||constructionModel.child_count!==57||constructionModel.PARTITION_OVERLAP_COUNT!==0||constructionModel.PARTITION_GAP_COUNT!==0||constructionModel.coverage_preservation_status!=='PASS')throw new Error('CONSTRUCTION_MODEL_INVALID');

const rt=await loadRegisteredRuntime('YKK AP','ウチリモ 内窓');
if(!rt?.sourcePackageIntegrity?.match||rt.sourcePackageIntegrity.actual!==exec38.current_runtime_manifest_sha256||rt.sourcePackageIntegrity.actual!==constructionModel.current_runtime_manifest_sha256)throw new Error('RUNTIME_IDENTITY_CHANGED');

const modelChildById=new Map(constructionModel.children.map(x=>[x.child_id,x]));
const slow=exec38.results.filter(x=>x.status==='NEEDS_FURTHER_PARTITIONING');
if(slow.length!==19||slow.some(x=>x.outcome!=='TIMEOUT_CONSTRAINT_ACTIVE'||x.constraint_start_valid!==true||x.constraint_progress_valid!==true||x.split_decision?.kind!=='VALUE'||x.split_decision?.value!=='wood'||x.constraints?.length!==4))throw new Error('SLOW_WOOD19_SET_INVALID');

const parents=[];
const children=[];
for(const source of slow){
  const sourceModel=modelChildById.get(source.child_id);
  if(!sourceModel||sourceModel.product_node!==source.product_node||sourceModel.lane_id!==source.lane_id)throw new Error('SLOW_WOOD19_MODEL_CHILD_MISSING:'+source.child_id);
  const prefix=stable(sourceModel.selector_prefix??{});
  ...
  const chosen=candidateProofs.find(x=>x.field_key===SPLIT&&x.status==='PASS');
  if(!chosen)throw new Error('WALL_SURFACE_SPLIT_NOT_SAFE:'+source.child_id);
  const vals=new Set(chosen.reachable_children.filter(x=>x.decision?.kind==='VALUE').map(x=>x.decision?.value));
  if(chosen.reachable_branch_count!==3||chosen.decision_domain_count!==3||!['yes','no','unknown'].every(v=>vals.has(v)))throw new Error('WALL_SURFACE_DOMAIN_UNEXPECTED:'+source.child_id);

  const parentId=`UHC-W19P-${hash({child_id:source.child_id,constraints:sourceConstraints}).slice(0,20)}`;
  parents.push({parent_id:parentId,source_child_id:source.child_id,lane_id:source.lane_id,product_node:source.product_node,selector_prefix:prefix,source_constraints:sourceConstraints,split_field:SPLIT,split_data_type:chosen.data_type,decision_domain_count:chosen.decision_domain_count,reachable_branch_count:chosen.reachable_branch_count,rejected_branch_count:chosen.rejected_branch_count,candidate_proofs:candidateProofs,partition_overlap_count:0,partition_gap_count:0,status:'PASS'});
  for(const reach of chosen.reachable_children){
    const constraints=[...sourceConstraints,{field_key:SPLIT,decision:reach.decision}];
    children.push({child_id:`UHC-W19C-${hash({parent_id:parentId,constraints}).slice(0,20)}`,parent_id:parentId,source_child_id:source.child_id,lane_id:source.lane_id,product_node:source.product_node,selector_prefix:prefix,constraints,split_field:SPLIT,split_decision:reach.decision,split_decision_key:dkey(reach.decision),selection_fingerprint:reach.selection_fingerprint,execution_status:'UNVERIFIED',status:'PLANNED_UNVERIFIED'});
  }
}
...
writeJson(`${OUT}/explicit-constraint-slow19-wood-wall-surface-partition-model-proof.json`,{schema_version:'1.0.0',artifact_type:'UCHIRIMO_SLOW19_WOOD_WALL_SURFACE_PARTITION_MODEL_PROOF',exact_head:head,source_exact_head:SH,source_run_id:Number(SR),source_artifact_name:SA,source_artifact_digest:SD,current_runtime_manifest_sha256:rt.sourcePackageIntegrity.actual,historical_selection_basis:{prior_depth5_observed_split_field:SPLIT,historical_evidence_reused_as_current_pass_evidence:false,current_19_parent_runtime_reproof_required:true},parent_count:parents.length,split_field:SPLIT,child_count:children.length,children_per_parent:3,PARTITION_OVERLAP_COUNT:overlap,PARTITION_GAP_COUNT:gap,parents,children,coverage_preservation_status:proofPass?'PASS':'FAIL',execution_performed:false,full_coverage_authorized:false,status:proofPass?'PASS':'BLOCKED'});
if(!proofPass)throw new Error('SLOW19_WOOD_WALL_SURFACE_PARTITION_MODEL_FAIL');

writeJson(`${OUT}/explicit-constraint-slow19-wood-wall-surface-partition-decision.json`,{schema_version:'1.0.0',artifact_type:'UCHIRIMO_SLOW19_WOOD_WALL_SURFACE_PARTITION_DECISION',exact_head:head,parent_count:19,child_count:57,split_field:SPLIT,PARTITION_OVERLAP_COUNT:0,PARTITION_GAP_COUNT:0,coverage_preservation_status:'PASS',execution_performed:false,next_recovery_action:'CALIBRATE_REPRESENTATIVE_WALL_SURFACE_CHILDREN_FOR_19_SLOW_WOOD_PARENTS',representative_execution_authorized:true,all_children_execution_authorized:false,slow19_wood_source_reexecution_authorized:false,remaining38_reexecution_authorized:false,full_constraint_execution_authorized:false,full_coverage_authorized:false,REQUESTED_CHANGE_SCOPE:'UCHIRIMO_HEAVY_RECOVERY_PARTITION_ONLY_19_SLOW_WOOD_CONSTRUCTION_CHILDREN_BY_WALL_SURFACE_FOR_REINFORCEMENT_AVAILABLE',REQUESTED_DIFF_COVERAGE:'PASS',UNREQUESTED_DIFF_COUNT:0,UCHIRIMO_FULL_COVERAGE_QA_GATE:'BLOCKED',UCHIRIMO_QA_STATUS:'UNVERIFIED',APP_INTEGRATION_READY:false,RELEASE_INPUT_GATE:'BLOCKED',status:'DIAGNOSTIC_COMPLETE'});

console.log('SLOW19_WOOD_WALL_SURFACE_MODEL_STATUS=PASS');
...
console.log('NEXT_RECOVERY_ACTION=CALIBRATE_REPRESENTATIVE_WALL_SURFACE_CHILDREN_FOR_19_SLOW_WOOD_PARENTS');
