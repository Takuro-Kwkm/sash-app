...
for(const p of wallPart.parents){
  ...
  if(sj(p.source_constraints)!==sj(source.constraints)||sj(p.source_constraints)!==sj(model.constraints)||sj(p.selector_prefix)!==sj(source.selector_prefix)||sj(p.selector_prefix)!==sj(model.selector_prefix)||p.lane_id!==source.lane_id||p.product_node!==source.product_node)throw new Error('WOOD_SOURCE_BINDING_MISMATCH:'+p.source_child_id);
