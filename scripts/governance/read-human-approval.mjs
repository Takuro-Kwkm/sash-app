...
readJson('artifacts/governance/human-flow-review.json');
...
for(let page=1;;page++){
 ...
 for(const row of rows){
  ...
  if(!a.reviewed_exact_head||!a.review_artifact_identity)continue;
  if(!latest||row.id>latest.id)latest={...a,id:row.id,url:row.html_url,updated_at:row.updated_at};
 }
 if(rows.length<100)break;
}
if(latest?.decision==='APPROVE')human={status:'PASS',reviewed_exact_head:latest.reviewed_exact_head,review_artifact_identity:latest.review_artifact_identity,human_approval_reference:latest.url,approved_at:latest.updated_at};
