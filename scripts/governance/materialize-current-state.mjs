...
// Connector snapshot is an auditable observation, never an assertion of a future live read.
let driveStatus='BLOCKED_LIVE_DRIVE_READ_NOT_CONFIGURED';
if(process.env.DRIVE_READONLY_ACCESS_TOKEN){
 for(const row of drive.entries){
  const r=await fetch(`https://www.googleapis.com/drive/v3/files/${row.drive_file_id}?fields=id,name,modifiedTime,version`,{headers:{Authorization:`Bearer ${process.env.DRIVE_READONLY_ACCESS_TOKEN}`}});
  if(!r.ok)throw Error(`DRIVE_READ_FAILED_${r.status}`);
  const live=await r.json();
  if(live.modifiedTime!==row.modified_identity || live.name!==row.name)throw Error(`DRIVE_AUTHORITY_CHANGED_${row.drive_file_id}`);
  row.live_revision=live.version;row.live_checked_at=new Date().toISOString();
 }
 driveStatus='PASS';
}
writeJson('artifacts/governance/drive-authority-observation.json',{...drive,exact_head:head,live_read_status:driveStatus});
...
const executions={
 ...
 'drive-authority-observation.json':['CURRENT_STATE_RECONSTRUCTION_GATE','Google Drive read-only live metadata verification',driveStatus==='PASS'?'PASS':'BLOCKED']
};
...
console.log(JSON.stringify({head,series:state.series_total,windows:state.base_window_count,fieldCoverage:coverage.status,reconstruction:state.governance_foundation.current_state_reconstruction_gate,human}));
