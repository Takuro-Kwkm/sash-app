...
const missingRequested=(changeScope.required_changed_files??[]).filter(p=>!changed.includes(p));
const scopeStatus=unrequested.length===0&&missingRequested.length===0?'PASS':'FAIL';
writeJson('artifacts/governance/scope-verification.json',{exact_head:head,base,task_id:changeScope.task_id,requested_change_scope:changeScope.requested_change_scope,changed_files:changed,requested_diff_coverage:missingRequested.length===0?'PASS':'FAIL',missing_requested_files:missingRequested,unrequested_diff_count:unrequested.length,unrequested_files:unrequested,repository_runtime_materialization_changed_files:changed.filter(p=>p.startsWith('src/catalog/runtime-master-packages/')),canonical_product_master_sources_read_only:true,product_master_mutation:0,status:scopeStatus});
if(unrequested.length)throw Error('UNREQUESTED_DIFF');
if(missingRequested.length)throw Error('REQUESTED_DIFF_COVERAGE_FAIL');
