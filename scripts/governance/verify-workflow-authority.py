...
pre=policy.get('controller_pre_human_jobs',{})
post=set(policy.get('controller_post_human_jobs',[]))
def deps_of(job):
 deps=job.get('needs',[])
 return [deps] if isinstance(deps,str) else (deps or [])
for name,d in docs.items():
 ...
 if name==controller:
  ...
  for key,j in d['jobs'].items():
   if key in pre:
    spec=pre[key]
    if set(deps_of(j))!=set(spec.get('needs',[])):errors.append(f'{name}/{key}: pre-human dependency mismatch')
    condition=str(j.get('if',''))
    for fragment in spec.get('required_if_contains',[]):
     if fragment not in condition:errors.append(f'{name}/{key}: pre-human condition mismatch')
    continue
   if key in post:
    if 'governance' not in deps_of(j) or "needs.governance.outputs.authorized == 'true'" not in str(j.get('if')):errors.append(f'{name}/{key}: controller bypass')
    continue
   errors.append(f'{name}/{key}: unregistered controller job')
  continue
...
report={'status':'FAIL' if errors else 'PASS','legacy_workflow_count':len(policy['legacy_workflows']),'legacy_independent_authority_count':sum(1 for n,d in docs.items() if n!=controller and set(d.get('on',d.get(True)) or {})!={'workflow_call'}),'controller_pre_human_jobs':sorted(pre),'controller_post_human_jobs':sorted(post),'errors':errors}
