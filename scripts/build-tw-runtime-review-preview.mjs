...
const [htmlSource, appSource, sizeSource, styles, waveStyles, engineSource] = await Promise.all([
  readFile(join(webRoot, 'index.html'), 'utf8'), readFile(join(webRoot, 'app.js'), 'utf8'),
  readFile(join(webRoot, 'size-presentation.js'), 'utf8'), readFile(join(webRoot, 'styles.css'), 'utf8'),
...
const sizeUrl=URL.createObjectURL(new Blob([${json(sizeSource)}],{type:"text/javascript"}));
const uiSource=${json(appSource)}.replace('from"/size-presentation.js"','from"'+sizeUrl+'"');
await import(URL.createObjectURL(new Blob([uiSource],{type:"text/javascript"})));
`;
