...
for(const row of assigned){
  ...
  async function visit(result,finalized,depth){
    ...
    if(windowVisited>MAX_TRAVERSAL_PER_WINDOW){
      ...
      traversalLimitFrontiers.push({
        frontier_id:`LIMIT-${hash(`${row.id}/${row.windowType}/${fp}`).slice(0,20)}`,
        manufacturer:row.manufacturer,series:row.series,product_id:row.id,window_type:row.windowType,
        field:field?.key??null,field_type:field?.dataType??null,candidate_count:field?enabled(field).length:0,
