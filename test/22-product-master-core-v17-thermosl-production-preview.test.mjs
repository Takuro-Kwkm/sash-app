...
test('v1.7 production preview binds exact formal Drive revision and approved Proposal',()=>{
  const p=preview();
...
test('v1.7 production preview maps 85 Size rows to exact 06_サイズ append range',()=>{
  const p=preview();
  const w=p.writePlan.writes[0];
  assert.equal(w.sheet,'06_サイズ');
  assert.equal(w.range,'A1562:V1646');
...
test('v1.7 production preview maps 85 glass rows to exact 08A append range',()=>{
  const p=preview();
  const w=p.writePlan.writes[1];
  assert.equal(w.sheet,'08A_サイズ別ガラス条件');
  assert.equal(w.range,'A1562:N1646');
  assert.equal(w.rows.length,85);
...
test('v1.7 production preview projects formal selectable Size count to 1495 and requires separate production approval',()=>{
  const p=preview();
...
test('v1.7 formal tail drift blocks preview',()=>{
