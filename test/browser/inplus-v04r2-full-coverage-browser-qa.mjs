...
const UI_SECTIONS = ['manufacturer','product','window_type','glass_spec','lowe_performance','spacer','cavity','body_color','frame_install','custom_size','options'];
const FORBIDDEN_FIELDS = ['screen','screen_type','screen_midrail','screen_net','exterior_color','interior_color','manufacturer','series','product_category','evidence','glass_config_id'];
...
async function runStateChunk(page, rows) {
  return page.evaluate(async ({ rows, uiSections, forbiddenFields }) => {
    ...
    for (const row of rows) {
      ...
      add(issues, JSON.stringify(mode?.values?.map((value) => value.value)) === JSON.stringify(['CUSTOM']), 'CUSTOM-only contract broken');
      ...
      results.push({ stateId:row.stateId, logicalCaseId:row.logicalCaseId, kind:row.kind, pass:issues.length === 0, dimensionStatus:result.dimensionResult?.status ?? null, validationStatus:result.validation?.status ?? null, overflow });
      if (issues.length) failures.push({ stateId:row.stateId, logicalCaseId:row.logicalCaseId, issues });
    }
    return { failures, results };
  }, { rows, uiSections:UI_SECTIONS, forbiddenFields:FORBIDDEN_FIELDS });
}
async function runViewport(name, contextOptions) {
  ...
  for (let start = 0; start < states.length; start += CHUNK_SIZE) {
    const chunkResult = await runStateChunk(page, states.slice(start, start + CHUNK_SIZE));
    failures.push(...chunkResult.failures);
    results.push(...chunkResult.results);
    if ((start + Math.min(CHUNK_SIZE, states.length - start)) % 300 < CHUNK_SIZE || start + CHUNK_SIZE >= states.length) console.log(`[${name}] ${Math.min(start + CHUNK_SIZE, states.length)}/${states.length} browser states verified`);
  }
...
try {
  ...
  const failedLogicalCases = new Set([...(report.desktop.failures ?? []).map((row) => row.logicalCaseId), ...(report.mobile.failures ?? []).map((row) => row.logicalCaseId)]);
  ...
  report.status = report.unverifiedQaCaseCount === 0 && report.desktop.status === 'PASS' && report.mobile.status === 'PASS' && report.consoleErrors.length === 0 && report.pageErrors.length === 0 && report.failedResponses.length === 0 ? 'PASS' : 'FAIL';
  await writeFile(`${OUT}/report.json`, JSON.stringify(report, null, 2));
  console.log(JSON.stringify({ status:report.status, baseWindowCount:report.baseWindowCount, logicalQaCaseCount:report.logicalQaCaseCount, browserStateChecksTotal:report.browserStateChecksTotal, verifiedQaCaseCount:report.verifiedQaCaseCount, unverifiedQaCaseCount:report.unverifiedQaCaseCount, desktop:report.desktop.status, mobile:report.mobile.status, consoleErrors:report.consoleErrors.length, pageErrors:report.pageErrors.length, failedResponses:report.failedResponses.length }, null, 2));
