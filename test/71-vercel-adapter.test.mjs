...
const EXPECTED_RUNTIME_IDS=new Set(['SER-LIX-EW','SER-LIXIL-TW','SER-YKKAP-UCHIRIMO','SER-YKKAP-INNOVEST']);
...
test('Vercel repository adapter preserves rewritten health path and Runtime identities', async()=>{
  ...
  assert.equal(health.runtimeMasterIntegrations.length,EXPECTED_RUNTIME_IDS.size);
  const byId=new Map(health.runtimeMasterIntegrations.map((row)=>[row.id,row]));
  assert.deepEqual(new Set(byId.keys()),EXPECTED_RUNTIME_IDS);
  ...
  assert.equal(byId.get('SER-YKKAP-INNOVEST').sourceHash,'6f47677023212228d1132407c9fce8ccd9d407c65fd19cbb9e1507f1845b287d');
});
...
test('Vercel repository adapter preserves rewritten Runtime integration route', async()=>{
  ...
  assert.equal(rows.length,EXPECTED_RUNTIME_IDS.size);
  assert.deepEqual(new Set(rows.map((row)=>row.id)),EXPECTED_RUNTIME_IDS);
