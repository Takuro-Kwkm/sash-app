...
test('20 pattern obeys size Master while Frost remains a formal manual-check sales candidate',()=>{assert.deepEqual(values('glass_type',{size:'SZ-S2H-00001'}),['CLEAR','FROST']);assert.deepEqual(values('glass_type',{size:'SZ-S2H-00093'}),['CLEAR','PATTERN','FROST']);const frost=field({size:'SZ-S2H-00001'},'glass_type').values.find((row)=>row.value==='FROST');assert.equal(frost.manualCheck,true);});
