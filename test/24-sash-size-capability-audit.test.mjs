...
test('full current gate has eight managed blocking PENDING and S2H proposal applied through external Change Control',()=>{
  const summary=read('summary.json');assert.deepEqual(summary.productMasterChangeProposals,[]);assert.deepEqual(summary.appliedProductMasterChangeProposals,['PMCP-LIX-SAMOSL-INNER-TILT-RANGE-20260903-001','PMCP-LIX-SAMOS2H-CUSTOM-DIMENSION-RULESET-20260903-001']);const pending=read('pending.json');assert.equal(pending.blockingCount,8);assert.ok(pending.resolved.some((row)=>row.id==='PEND-SIZE-S2H-CUSTOM-001'));assert.equal(pending.items.some((row)=>row.id==='PEND-SIZE-S2H-CUSTOM-001'),false);
  const gate=buildSizeCapabilityAuditGate({summary,standardAudit:read('standard-size-audit.json'),customAudit:read('custom-capability-audit.json'),pending,ruleAudits:[read('dimension-rule-audit-thermosl.json'),read('dimension-rule-audit-apw431.json')],proposals:[]});assert.equal(gate.integrityGate,'PASS');assert.equal(gate.status,'PARTIAL_PASS');assert.equal(gate.blockingPending,8);assert.equal(gate.proposalCount,0);assert.equal(gate.proposalApprovalGate,'NO_PROPOSAL_PENDING');
