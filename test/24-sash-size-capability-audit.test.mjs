...
test('full current gate has seven managed blocking PENDING, applied S2H, and one APW430 HUMAN proposal gate',()=>{
  const summary=read('summary.json');const apw430Proposal=readAPW430Proposal();assert.deepEqual(summary.productMasterChangeProposals,['PMCP-YKK-APW430-CUSTOM-DIMENSION-RULESET-20260903-001']);assert.deepEqual(summary.appliedProductMasterChangeProposals,['PMCP-LIX-SAMOSL-INNER-TILT-RANGE-20260903-001','PMCP-LIX-SAMOS2H-CUSTOM-DIMENSION-RULESET-20260903-001']);const pending=read('pending.json');assert.equal(pending.blockingCount,7);assert.ok(pending.resolved.some((row)=>row.id==='PEND-SIZE-S2H-CUSTOM-001'));assert.ok(pending.resolved.some((row)=>row.id==='PEND-SIZE-APW430-CUSTOM-001'));assert.equal(pending.items.some((row)=>row.id==='PEND-SIZE-S2H-CUSTOM-001'),false);assert.equal(pending.items.some((row)=>row.id==='PEND-SIZE-APW430-CUSTOM-001'),false);
  const gate=buildSizeCapabilityAuditGate({summary,standardAudit:read('standard-size-audit.json'),customAudit:read('custom-capability-audit.json'),pending,ruleAudits:[read('dimension-rule-audit-thermosl.json'),read('dimension-rule-audit-apw431.json')],proposals:[apw430Proposal]});assert.equal(gate.integrityGate,'PASS');assert.equal(gate.status,'PARTIAL_PASS');assert.equal(gate.blockingPending,7);assert.equal(gate.proposalCount,1);assert.equal(gate.proposalApprovalGate,'HUMAN_APPROVAL_PENDING');
  const id='PMCP-LIX-SAMOS2H-CUSTOM-DIMENSION-RULESET-20260903-001';const approval=readJson(`data/master-change-control/approvals/${id}.approval.json`),staging=readJson(`data/master-change-control/applied/${id}.staging.json`),prodApproval=readJson(`data/master-change-control/production-approvals/${id}.production-approval.json`),production=readJson(`data/master-change-control/production/${id}.applied.json`),runtime=readJson('data/master-change-control/runtime/SAMOS2H_CUSTOM_RUNTIME_REGENERATION_V10.json');assert.equal(approval.approverType,'HUMAN');assert.equal(staging.proposalStatus,'APPLIED');assert.equal(prodApproval.productionApproval,true);assert.equal(production.status,'PRODUCTION_APPLY_COMPLETE');assert.equal(production.formalTarget.postWriteSha256,'7ca8f5cca19187bfb841bc3f3393fb29de591dc554714627faf3a652130cd8a7');assert.equal(production.postWriteReadback.dimensionRuleCount,17);assert.equal(production.postWriteReadback.unexpectedChangedExistingSheets,0);assert.equal(runtime.status,'RUNTIME_REGENERATION_COMMITTED');assert.equal(runtime.runtimeVersion,'v1.0');assert.equal(runtime.runtimeProjection.dimensionRuleCount,17);assert.equal(runtime.runtimeProjection.directManufacturerValueEditToGenericCore,false);
});
